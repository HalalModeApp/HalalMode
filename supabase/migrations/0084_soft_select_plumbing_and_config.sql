-- Carry the soft-select count through to the scorer, and name the two knobs.
--
-- The count lives on pair_exposure, but the planner reads a per-run snapshot,
-- so a column that stops at the snapshot boundary is one the ranking cannot
-- see. Same journey explicit_pass_count made in 0075, for the same reason.
...
comment on column halal_mode_private.matching_run_candidate_snapshots.soft_select_count is
  'Times this pair was read at length but left unkept. Lifts the pair prior by soft_select_lift, capped at the value a never-shown pair would have.';
...
begin
  ...
  select
    ...
    coalesce(pe.soft_select_count, 0)
  from halal_mode_private.matching_run_member_snapshots m
...
create function public.matching_candidate_edges_service(
  ...
  pair_soft_select_count integer
)
language plpgsql
stable
security definer
...
-- Configuration
--
-- Thirty days after a first pass, against a ceiling of twenty-one for an
-- ordinary repeat, so a pass always costs more time than simply not being
-- chosen. The validator enforces that relationship rather than the number.
...
select
  ...
  'A first pass costs a month; soft select lifts a pair that was read but unkept.',
  now();
