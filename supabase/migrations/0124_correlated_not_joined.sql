-- Correlated subqueries, not joins. This is the ninety seconds.
--
-- 0121 split the OR'd safety vetoes into separate checks, which was right, and
-- wrote them as joins, which was wrong — and left validation exactly as slow,
-- so the split looked like it had failed when only half of it had.
--
-- `join public.profiles p on p.id in (e.a, e.b)` cannot use the primary key.
-- It scans every profile for every edge and evaluates the consent function on
-- each row it touches: with 74 edges and a few hundred profiles that is tens of
-- thousands of function calls. The same condition as a correlated `exists` is
-- two keyed lookups.
--
-- So the rule is both halves together: ask each safety question separately,
-- and ask it as a correlated subquery. Measured that way over the same 74
-- edges, the seven checks are 1-5ms each.
--
-- Instrumenting validation statement by statement is what settled it, after
-- two rewrites made on inspection did not. Everything else in the function is
-- unchanged from 0122.
...
begin
  ...
  begin
    -- Split for the same reason as the vetoes below: mixing plain arithmetic
    -- with a correlated `not exists` under OR stops the anti-join using the
    -- candidate snapshot's primary key. The bounds are pure arithmetic over the
    -- payload and need no table at all; membership is an anti-join and wants
    -- its index. Together they were quadratic, apart they are milliseconds.
    if exists (
      select 1
      from jsonb_to_recordset(p_edges) as e(a uuid, b uuid, score numeric, utility numeric)
    ...
    if exists (
      ...
      left join halal_mode_private.matching_run_candidate_snapshots c
       ...
       and c.user_low = least(e.a, e.b)
       and c.user_high = greatest(e.a, e.b)
    ...
    --
    -- Separate checks, each a correlated `exists`. Both parts of that matter
    -- and were learned the hard way. As one query with the conditions OR'd
    -- together the planner cannot use any of the indexes each branch was
    -- written for. Rewritten as joins — `join profiles on p.id in (e.a, e.b)` —
    -- it is worse still, because that scans every profile for every edge and
    -- calls the consent function on each one. As a correlated subquery the same
    -- condition is two primary-key lookups.
    --
    -- Measured over the same 74 edges: 1-5ms each, against 115 seconds for the
    -- single OR'd query they replace.
    if exists (
