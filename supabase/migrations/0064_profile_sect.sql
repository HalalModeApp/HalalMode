-- Let a member declare their sect.
--
-- 0060 added the column and the preference side; this is the write path. The
-- profile RPC from 0045 is the only way a member may change their own profile,
-- with an explicit allowlist, so without this the field is unreachable and a
-- sect preference would match nobody.
--
-- Restated whole for the same reason as 0063: an allowlist entry and its
-- assignment have to move together.
...
begin
  ...
  where key not in (
    ...
    'audio_greeting_url', 'audio_duration_seconds', 'sect'
  )
  limit 1;
