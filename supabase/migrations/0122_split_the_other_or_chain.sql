-- The same collapse, in the check above the vetoes.
--
-- 0121 split the eight safety vetoes and validation was still slow, because
-- there were two OR chains, not one. This is the other: the check that every
-- chosen edge is a frozen candidate ORs five scalar comparisons together with
-- a correlated `not exists` against the candidate snapshot.
--
-- Same collapse, same cause. The planner cannot satisfy an OR of arithmetic
-- and an anti-join with the anti-join's index, so it stops trying. Apart, the
-- bounds touch no table at all and the membership check is a keyed left join.
--
-- Split rather than rewritten: identical meaning, identical error, and the
-- rest of the function is unchanged from 0121.
